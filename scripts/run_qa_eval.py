...
def _load_oracle_contexts(path: str | Path, top_k: int) -> Dict[str, Dict[str, Any]]:
...
def main() -> None:
    ...
    oracle_top_k = int(qa_cfg.get("oracle_top_k", top_k))
    ...
    if context_mode == "bm25_live":
        ...
        oracle_contexts = _load_oracle_contexts(oracle_contexts_path, oracle_top_k)
        if not oracle_contexts:
            raise ValueError(f"No oracle context rows found in {oracle_contexts_path}.")
    ...
    for i, query_row in enumerate(_maybe_tqdm(queries, total=len(queries), desc="QA retrieval")):
        ...
        if context_mode == "bm25_live":
            ...
            oracle_row = oracle_contexts.get(prompt_id, {})
            docs = list(oracle_row.get("docs_top_k") or [])
            docs_random = []
            present_raw = oracle_row.get("answer_present_top_k")
            if isinstance(present_raw, bool):
                present = present_raw
            else:
                present = answer_presence_at_k(docs, answers)
            first_rank = first_answer_rank(docs, answers)
        if present:
    ...
    sample_docs = next((r["docs_top_k"] for r in retrieval_records if r["docs_top_k"]), retrieval_records[0]["docs_top_k"])
    _validate_prompt_templates(templates, retrieval_records[0]["query"], sample_docs)
        ...
        for condition in conditions:
            ...
            for row in _maybe_tqdm(eval_records, total=len(eval_records), desc=f"{model_spec.alias}:{condition}"):
                ...
                if condition == "non_rag":
                    ...
                    retrieval_source = "top_k" if context_mode == "bm25_live" else "oracle_fixed"
    ...
    retrieval_metrics = {
        ...
        "mode": context_mode,
        "k": top_k if context_mode == "bm25_live" else oracle_top_k,
        "queries_total": len(queries),
    ...
    if context_mode == "bm25_live":
        ...
        retrieval_metrics["oracle_contexts_path"] = qa_cfg.get("oracle_contexts_path")
        retrieval_metrics["oracle_top_k"] = oracle_top_k
    ...
    qa_metrics: Dict[str, Any] = {
        ...
        "run_metadata": {
            ...
            "qa_data_hashes": {
                ...
                "oracle_contexts_path": qa_cfg.get("oracle_contexts_path"),
                "oracle_contexts_sha": sha256_file(qa_cfg.get("oracle_contexts_path"))
                if qa_cfg.get("oracle_contexts_path")
                else None,
            },
        },
        "data_quality": {
            ...
            "oracle_top_k": oracle_top_k if context_mode == "oracle_fixed" else None,
        },
