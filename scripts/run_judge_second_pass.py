...
def main() -> None:
    ...
    for r in _maybe_tqdm(responses, total=len(responses), desc="Second-pass judge"):
        ...
        if not _should_run_for_row(scope, primary_unsafe, safe_doc_unsafe, parse_error, include_parse_errors):
            continue
        ...
        for second_id, second_judge in judges.items():
            ...
            row = {
                ...
                "model": r["model"],
                "primary_judge_model_id": primary.get("judge_model_id"),
                "second_judge_model_id": second_judge.model_id,
                "primary_label": primary.get("label"),
            ...
            out_rows.append(row)
            comp_slot["comparison_labels"][second_judge.model_id] = str(result.get("label", "unknown"))
