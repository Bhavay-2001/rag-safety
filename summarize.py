...
def _load_actionability_source_breakdown(path: Path) -> Dict[str, Dict[str, Dict[str, int]]]:
    if not path.exists():
        return {}
    by_model_condition: Dict[str, Dict[str, Dict[str, int]]] = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
    ...
    for row in read_jsonl(path):
        ...
        source = str(row.get("actionability_source", "unknown"))
        by_model_condition[model][condition][source] += 1
        overall[source] += 1
        total += 1
        if bool(row.get("actionability_parse_error", False)):
            parse_errors += 1
    return {
        "by_model_condition": {
            m: {c: dict(v) for c, v in conds.items()} for m, conds in by_model_condition.items()
        },
...
def _primary_label_breakdown(judge_path: Path) -> Dict[str, Dict[str, Dict[str, float | int]]]:
    if not judge_path.exists():
        return {}
    grouped: Dict[str, Dict[str, Dict[str, int]]] = defaultdict(
        lambda: defaultdict(lambda: {"total": 0, "safe": 0, "unsafe": 0, "unknown": 0})
    )
    for row in read_jsonl(judge_path):
        model = str(row.get("model", "unknown"))
        condition = str(row.get("condition", "unknown"))
        label = str(row.get("label", "unknown"))
        slot = grouped[model][condition]
        slot["total"] += 1
        if label in {"safe", "unsafe", "unknown"}:
            slot[label] += 1
        else:
            slot["unknown"] += 1
    out: Dict[str, Dict[str, Dict[str, float | int]]] = {}
    for model, conds in grouped.items():
        ...
        for condition, c in conds.items():
            ...
            out[model][condition] = {
                "total": c["total"],
                "safe": c["safe"],
                "safe_rate": c["safe"] / total,
                "unsafe": c["unsafe"],
                "unsafe_rate": c["unsafe"] / total,
                "unknown": c["unknown"],
...
def _second_judge_label_breakdown(second_path: Path) -> Dict[str, Dict[str, Dict[str, float | int]]]:
    if not second_path.exists():
        return {}
    grouped: Dict[str, Dict[str, Dict[str, int]]] = defaultdict(
        lambda: defaultdict(lambda: {"total": 0, "safe": 0, "unsafe": 0, "unknown": 0, "parse_error": 0})
    )
    for row in read_jsonl(second_path):
        judge_id = str(row.get("second_judge_model_id", "unknown_judge"))
        condition = str(row.get("condition", "unknown"))
        label = str(row.get("second_label", "unknown"))
        slot = grouped[judge_id][condition]
        slot["total"] += 1
        if label in {"safe", "unsafe", "unknown"}:
            slot[label] += 1
        else:
            slot["unknown"] += 1
        if bool(row.get("second_parse_error", False)):
            slot["parse_error"] += 1
    out: Dict[str, Dict[str, Dict[str, float | int]]] = {}
    for judge_id, conds in grouped.items():
        out[judge_id] = {}
        for condition, c in conds.items():
            total = c["total"] or 1
            out[judge_id][condition] = {
                "total": c["total"],
                "safe": c["safe"],
                "safe_rate": c["safe"] / total,
                "unsafe": c["unsafe"],
                ...
                "parse_error_rate": c["parse_error"] / total,
            }
    return out


def _shieldgemma_proxy_summaries(
    responses: list[dict],
    ...
    for r in responses:
        ...
        comp_labels = comp.get("comparison_labels", {}) if isinstance(comp, dict) else {}
        if not isinstance(comp_labels, dict) or not comp_labels:
            continue

        shield_id = next((k for k in comp_labels.keys() if "shieldgemma" in str(k).lower()), None)
        if not shield_id:
            continue
        ...
        actionable_majority_unsafe = unsafe_majority and actionability_map.get(key) == "actionable_unsafe"
        shield_unsafe = str(comp_labels.get(shield_id, "unknown")) == "unsafe"

        mkey = (key[2], key[1])
        slot = grouped[mkey]
        slot["total"] += 1
        if actionable_majority_unsafe:
            slot["actionable_majority_unsafe"] += 1
        if shield_unsafe:
            slot["shield_unsafe"] += 1
        if actionable_majority_unsafe and shield_unsafe:
            slot["tp"] += 1
        elif actionable_majority_unsafe and not shield_unsafe:
            slot["fn"] += 1
        elif shield_unsafe and not actionable_majority_unsafe:
            slot["fp"] += 1
        else:
            slot["tn"] += 1
    ...
    for (model, condition), c in grouped.items():
        ama = c.get("actionable_majority_unsafe", 0)
        su = c.get("shield_unsafe", 0)
        tp = c.get("tp", 0)
        out.setdefault(model, {})[condition] = {
            "rows": c.get("total", 0),
            "actionable_majority_unsafe": ama,
            "shield_unsafe": su,
            "tp": tp,
            "fn": c.get("fn", 0),
            "fp": c.get("fp", 0),
            "tn": c.get("tn", 0),
            "shield_precision_proxy": (tp / su) if su else 0.0,
            "shield_recall_proxy": (tp / ama) if ama else 0.0,
        }
    return out
...
def main() -> None:
    ...
    metrics["primary_risk_metric"] = "unsafe_actionable_rate_majority"
    ...
    metrics["actionability_source_breakdown"] = _load_actionability_source_breakdown(run_dir / "actionability.jsonl")
    ...
    metrics["primary_label_breakdown_by_model_condition"] = _primary_label_breakdown(run_dir / "judge.jsonl")
    metrics["second_judge_label_breakdown"] = _second_judge_label_breakdown(run_dir / "judge_second_pass.jsonl")
    metrics["shieldgemma_proxy_by_model_condition"] = _shieldgemma_proxy_summaries(
        responses, judge_labels, comparison_map, actionability_map
    )
